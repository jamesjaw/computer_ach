...
int hit(int way,cach* set,string add){
    for(int i=0;i<way;i++){
        if(set[i].tag == add){
            return 1;
        }
...
int main(int argc,char* argv[]){
    ...
    cach** my_cach = new cach*[cache_sets + 1];
    for(int i=0;i<cache_sets;i++){
        my_cach[i] = new cach[associativity + 1];
    }
...
//cout<<"totla set: "<<set_num<<"\n";
    ...
    cach** new_cach = new cach*[cache_sets + 1];
    for(int i=0;i<cache_sets;i++){
        new_cach[i] = new cach[associativity + 1];
    }
    
    for(int i=0;i<cache_sets;i++){
        for(int j=0;j<associativity;j++){
            new_cach[i][j].NRU_bit = 1;
            new_cach[i][j].tag = " ";
        }
    }
    ...
    for(int i=0;i<p_count;i++){
        ...
        if(hit(associativity, new_cach[set], tag)){
            hitornot.push_back(1);
        }
        else{
            hitornot.push_back(0);
            miss++;
        }
    }
